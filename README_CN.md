...
- 保存在限定 Markdown 区域内的一次性任务，以及会按本地日期自动刷新的每日、每周任务；可隐藏归档入口，并支持在未完成、已完成和已归档任务列表内长按拖拽排序。
- 支持跨午夜时段的每日、每周计划模板。
- 手动维护的文件分组，可在主页和设置页中长按拖拽，在组内排序或跨组移动，并支持重命名跟随和失效恢复。
...
设置页可以配置界面语言、主题和外观、Banner 来源、模块可见性与顺序、热力图偏好、日记源、任务源与归档入口、计划模板、文件分组和插件数据重置。
...
- 任务源是一个带有 Homepage 管理区边界的 Markdown 文件；一次性任务可归档，周期任务长期保留在活动区，并直接在文件中保存周期状态。拖拽后的任务顺序会直接写回这个管理区。
- 文件分组只包含用户明确添加的仓库文件路径，并保留手动排列的分组和文件顺序；插件不会建立全仓库内容索引。
